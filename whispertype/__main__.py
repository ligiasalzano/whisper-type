...
Dependências do sistema:
    ...
    Linux:   sudo apt install ffmpeg pulseaudio-utils xdotool xclip   # X11
             sudo apt install ffmpeg pulseaudio-utils ydotool wl-clipboard  # Wayland
"""
...
def _suppress_terminal_echo() -> None:
    """
    Desabilita o echo do terminal e redireciona stdin para /dev/null.
    Deve ser chamado ANTES de importar o pynput, pois ele abre a
    conexão com o terminal na importação — depois é tarde demais.
    """
    import termios
    ...
    try:
        ...
        attrs[3] &= ~termios.ECHO   # desliga ECHO
        attrs[3] &= ~termios.ICANON # desliga modo canônico (evita buffering)
        termios.tcsetattr(fd, termios.TCSADRAIN, attrs)
    except Exception:
        pass  # terminal pode não suportar (ex: pipe), ignora silenciosamente

    # Redireciona stdin para /dev/null
...
def _run_linux(model: whisper.Whisper) -> None:
    # ── IMPORTANTE: suprimir echo ANTES de importar pynput ──────────
    _suppress_terminal_echo()
    ...
    # Verifica dependências de clipboard e colagem conforme a sessão
    if session == "wayland":
        _check_dependency("ydotool",  "Instale com: sudo apt install ydotool")
        _check_dependency("wl-copy",  "Instale com: sudo apt install wl-clipboard")
    else:
        _check_dependency("xdotool", "Instale com: sudo apt install xdotool")
        if not shutil.which("xclip") and not shutil.which("xsel"):
            log.error("xclip ou xsel não encontrado. Instale com: sudo apt install xclip")
            sys.exit(1)
